...
// This intermediate class is used to enable UIButton views in storyboards to have fonts and backgrounds determined by TidepoolMobileStyles data.

@IBDesignable class TidepoolMobileSimpleUIButton: TPUIButton {
...
@IBDesignable class TidepoolMobileUIButton: UIButton {
        ...
        if let (font, textColor) = Styles.usageToFontWithColor[usage] {
            if let titleLabel = titleLabel {
                ...
                if let titleStr = titleLabel.text {
                    let disabledTitle = NSAttributedString(string: titleStr, attributes:[NSAttributedStringKey.font: titleLabel.font, NSAttributedStringKey.foregroundColor: Styles.dimmedWhiteColor])
